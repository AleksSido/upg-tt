	...
	reducers: {
		toggleEventGenreId: (state, action: PayloadAction<EventGenreId>) => {
			if (state.genreId.includes(action.payload)) {
				const index = state.genreId.findIndex(
...
export const { toggleEventGenreId, resetEventGenreId, updateEventKeyword } =
	eventListRequestSlice.actions;

export const selectEventGenreId = (state: RootState) =>
	state.eventListRequest.genreId;
export const selectEventKeyword = (state: RootState) =>
	state.eventListRequest.keyword;
